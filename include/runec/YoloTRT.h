...
public:
  YoloTRT(
    std::string user_wts_name = "model.wts", std::string user_engine_name = "model.engine",
    std::string user_class_file = "classes.txt", std::string user_sub_type = "n");
  ~YoloTRT();
  bool setConfig(
    std::string InputName = "images", std::string OutputName = "output",
...
private:
  void serializeEngine(
    const int & kBatchSize, std::string & wts_name, std::string & engine_name,
    std::string & sub_type);
  std::string wts_name = "";

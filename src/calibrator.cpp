  ...
  if ((dir = opendir(path.c_str())) != nullptr) {
    while ((ent = readdir(dir)) != nullptr) {
      if (strcmp(ent->d_name, ".") != 0 && strcmp(ent->d_name, "..") != 0) {
        files.push_back(path + "/" + ent->d_name);          // 添加文件路径
      }
    }
    closedir(dir);
  } else {
...
Calibrator::Calibrator(
  int batchsize, int input_w, int input_h, std::string img_dir,
...
const void * Calibrator::readCalibrationCache(size_t & length) noexcept
{
  ...
  if (READCACHE && input.good()) {
    std::copy(
      ...
      std::back_inserter(CALIBRATORCACHE));
  }
